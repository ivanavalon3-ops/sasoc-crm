...
const TT={contentStyle:{background:"#0f172a",border:"1px solid rgba(255,255,255,.1)",borderRadius:8,color:"#0f172a",fontSize:12},labelStyle:{color:"#64748b"}};

// ─── SHARED UI ────────────────────────────────────────────────────
const IS = {background:"#f1f5f9",border:"1px solid rgba(255,255,255,.1)",borderRadius:8,padding:"8px 11px",color:"#0f172a",fontSize:13,width:"100%",boxSizing:"border-box",fontFamily:"inherit"};
  ...
  return (
    <div style={{gridColumn:s2?"span 2":"span 1",display:"flex",flexDirection:"column",gap:4}}>
      <div style={{display:"flex",alignItems:"center",gap:6}}>
        ...
        {hint&&<span style={{fontSize:10,color:"#dbeafe",background:"#dbeafe",padding:"1px 6px",borderRadius:20}}>opcional</span>}
      </div>
      {children}
    </div>
  );
...
function Bdg({t,sm}){
  const m={Activo:["#065f46","#059669"],Cobrado:["#065f46","#059669"],Pagado:["#065f46","#059669"],Depositado:["#065f46","#059669"],Cerrado:["#065f46","#059669"],Ganado:["#065f46","#059669"],"A cobrar":["#065f46","#059669"],Inactivo:["#78350f","#fcd34d"],Parcial:["#78350f","#fcd34d"],Seguimiento:["#78350f","#fcd34d"],"Sin respuesta":["#78350f","#fcd34d"],"Ret. IIBB":["#78350f","#fcd34d"],"A pagar":["#7f1d1d","#fca5a5"],NC:["#7f1d1d","#fca5a5"],Vencido:["#7f1d1d","#fca5a5"],Rechazado:["#7f1d1d","#fca5a5"],Suspendido:["#7f1d1d","#fca5a5"],Pendiente:["#dbeafe","#93c5fd"],"En custodia":["#dbeafe","#93c5fd"],Prospecto:["#0f172a","#64748b"],Perdido:["#7f1d1d","#fca5a5"],Anulado:["#0f172a","#64748b"]};
  const[bg,co]=m[t]||["#0f172a","#64748b"];
...
function Av({name,size=38}){
  ...
  const c=["#065f46","#dbeafe","#78350f","#4c1d95","#164e63"];
...
function Mdl({title,onClose,children,wide}){
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,padding:20}}>
      <div style={{background:"#ffffff",border:"1px solid rgba(255,255,255,.1)",borderRadius:16,width:"100%",maxWidth:wide?860:640,maxHeight:"92vh",overflowY:"auto",boxShadow:"0 24px 64px rgba(0,0,0,.6)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"17px 22px",borderBottom:"1px solid rgba(255,255,255,.08)",position:"sticky",top:0,background:"#ffffff",zIndex:10,borderBottom:"1px solid #f1f5f9"}}>
...
function Cfm({msg,onOk,onCancel}){
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200}}>
      <div style={{background:"#ffffff",border:"1px solid rgba(255,255,255,.1)",borderRadius:14,padding:28,maxWidth:360,width:"90%",textAlign:"center"}}>
        <p style={{color:"#0f172a",fontSize:15,marginBottom:22}}>{msg}</p>
        <div style={{display:"flex",gap:10,justifyContent:"center"}}>
          <button onClick={onCancel} style={{padding:"8px 22px",borderRadius:8,border:"1px solid rgba(255,255,255,.12)",background:"transparent",color:"#64748b",cursor:"pointer"}}>Cancelar</button>
...
function Tst({msg,type}){
  const ok=type!=="error";
  return <div style={{position:"fixed",bottom:28,right:28,background:ok?"#059669":"#ef4444",color:ok?"#059669":"#fca5a5",border:`1px solid ${ok?"#10b981":"#ef4444"}`,borderRadius:10,padding:"11px 18px",fontSize:13,fontWeight:700,zIndex:300,boxShadow:"0 8px 24px rgba(0,0,0,.4)"}}>{ok?"✓ ":"✕ "}{msg}</div>;
}

function Met({label,val,sub,up,dn}){
  return (
    <div style={{background:"#ffffff",borderRadius:10,padding:"14px 16px",border:"1px solid rgba(255,255,255,.07)"}}>
...
function Crd({children,mb}){
  return <div style={{background:"#ffffff",border:"1px solid rgba(255,255,255,.07)",borderRadius:14,padding:"18px 20px",marginBottom:mb??16}}>{children}</div>;
}
...
function Th({children}){return <th style={{textAlign:"left",padding:"7px 10px",fontSize:10,fontWeight:700,color:"#94a3b8",textTransform:"uppercase",letterSpacing:".5px",borderBottom:"1px solid rgba(255,255,255,.07)",whiteSpace:"nowrap"}}>{children}</th>;}
function Td({children,g,r,mono,fw}){return <td style={{padding:"9px 10px",fontSize:13,color:g?VERDE:r?ROJO:"#94a3b8",fontWeight:fw?600:400,fontFamily:mono?"monospace":"inherit",borderBottom:"1px solid rgba(255,255,255,.04)",whiteSpace:"nowrap"}}>{children}</td>;}

const btnPri = {padding:"8px 18px",borderRadius:8,border:"1px solid #059669",background:"#059669",color:"#ffffff",cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:"inherit"};
const btnSec = {padding:"8px 16px",borderRadius:8,border:"1px solid rgba(255,255,255,.12)",background:"transparent",color:"#64748b",cursor:"pointer",fontSize:13,fontFamily:"inherit"};
...
function BtnExport({ onClick, label = "Exportar Excel" }) {
  return (
    <button onClick={onClick} style={{padding:"7px 14px",borderRadius:8,border:"1px solid rgba(255,255,255,.12)",background:"#f1f5f9",color:"#64748b",cursor:"pointer",fontSize:12,fontWeight:600,fontFamily:"inherit",display:"flex",alignItems:"center",gap:6}}>
      ↓ {label}
    </button>
  );
}
...
function PageDashboard(){
  ...
  return (
    <div>
      ...
      {alertas>0&&(
        <div style={{background:"#fef2f2",border:"1px solid rgba(248,113,113,.2)",borderRadius:12,padding:"12px 16px",marginBottom:16,display:"flex",alignItems:"center",gap:10}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"3fr 2fr",gap:14,marginBottom:16}}>
        <Crd mb={0}>
          <p style={{color:"#0f172a",fontWeight:700,fontSize:14,marginBottom:14}}>Facturación mensual</p>
          {chart.length>0?(
          ...
          ):<p style={{color:"#94a3b8",fontSize:13,textAlign:"center",padding:"40px 0"}}>Cargá ventas para ver el gráfico</p>}
        </Crd>
        <Crd mb={0}>
          <p style={{color:"#0f172a",fontWeight:700,fontSize:14,marginBottom:14}}>Top clientes</p>
          {topC.length>0?topC.map(([nom,mon])=>(
            <div key={nom} style={{marginBottom:10}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                <span style={{fontSize:12,color:"#94a3b8",maxWidth:130,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{nom}</span>
          ...
          )):<p style={{color:"#94a3b8",fontSize:13,textAlign:"center",padding:"20px 0"}}>Sin datos aún</p>}
        </Crd>
      </div>
...
function FormClienteExtra({empresa,initial,onSave,onClose}){
  ...
  return(
    <div>
      <div style={{padding:"12px 16px",background:"#ecfdf5",border:"1px solid rgba(16,185,129,.15)",borderRadius:10,marginBottom:18,display:"flex",alignItems:"center",gap:12}}>
        <Av name={empresa} size={40}/><div><p style={{color:"#0f172a",fontWeight:700,fontSize:14,margin:0}}>{empresa}</p><p style={{color:"#64748b",fontSize:12,margin:"3px 0 0"}}>Todos los campos son opcionales</p></div>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:13}}>
        ...
        <Fld label="Dirección">
          <div style={{display:"flex",gap:8}}>
            <button type="button" onClick={()=>set("esEgreso",false)} style={{flex:1,padding:"8px",borderRadius:8,border:`1px solid ${!f.esEgreso?VERDE:"#e2e8f0"}`,background:!f.esEgreso?"#d1fae5":"transparent",color:!f.esEgreso?VERDE:"#64748b",cursor:"pointer",fontSize:13,fontWeight:600}}>↓ Ingreso</button>
            <button type="button" onClick={()=>set("esEgreso",true)} style={{flex:1,padding:"8px",borderRadius:8,border:`1px solid ${f.esEgreso?"#ef4444":"#e2e8f0"}`,background:f.esEgreso?"#fef2f2":"transparent",color:f.esEgreso?ROJO:"#64748b",cursor:"pointer",fontSize:13,fontWeight:600}}>↑ Egreso</button>
          </div>
        </Fld>
...
function FormProyeccion({movs,onSave,onClose}){
  ...
  return(
    <div>
      ...
      {items.map((it,idx)=>(
        <div key={it.id} style={{background:"#f8fafc",borderRadius:10,padding:14,marginBottom:10,border:"1px solid rgba(255,255,255,.06)"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr auto",gap:10,alignItems:"end"}}>
            ...
            <div style={{display:"flex",gap:6,paddingBottom:1}}>
              <button type="button" onClick={()=>setI(it.id,"esEgreso",false)} style={{padding:"8px 10px",borderRadius:7,border:`1px solid ${!it.esEgreso?VERDE:"#e2e8f0"}`,background:!it.esEgreso?"#d1fae5":"transparent",color:!it.esEgreso?VERDE:"#64748b",cursor:"pointer",fontSize:12,fontWeight:600}}>↓</button>
              <button type="button" onClick={()=>setI(it.id,"esEgreso",true)} style={{padding:"8px 10px",borderRadius:7,border:`1px solid ${it.esEgreso?"#ef4444":"#e2e8f0"}`,background:it.esEgreso?"#fef2f2":"transparent",color:it.esEgreso?ROJO:"#64748b",cursor:"pointer",fontSize:12,fontWeight:600}}>↑</button>
            </div>
          </div>
        </div>
      ))}
...
function FormComisionesConfig({initial,vendedores,onSave,onClose}){
  ...
  return(
    <div>
      ...
      {allVnd.length===0&&<p style={{color:"#94a3b8",fontSize:13,textAlign:"center",padding:"20px 0"}}>No hay vendedores registrados. Cargá ventas con el campo Vendedor completo.</p>}
      {allVnd.map(vnd=>(
        <div key={vnd} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:"1px solid rgba(255,255,255,.06)"}}>
          <Av name={vnd} size={36}/>
          <div style={{flex:1}}><p style={{fontSize:13,fontWeight:600,color:"#0f172a",margin:"0 0 2px"}}>{vnd}</p><p style={{fontSize:11,color:"#475569",margin:0}}>Vendedor</p></div>
      ...
      <div style={{marginTop:16,padding:"12px 14px",background:"#f8fafc",borderRadius:10,border:"1px solid rgba(255,255,255,.06)"}}>
    ...
    <div>
      ...
      {costos.length>0&&(
        <div style={{marginTop:16,padding:"12px 14px",background:"#f8fafc",borderRadius:10,border:"1px solid rgba(255,255,255,.06)"}}>
          ...
          {costos.map((c,i)=>{const p=provs.find(x=>x.id===c.proveedorId);return<div key={i} style={{display:"flex",justifyContent:"space-between",fontSize:12,color:"#64748b",marginBottom:4}}><span>{p?.nombre||"Proveedor"} — {c.productos||"S/D"}</span><span style={{color:ROJO,fontWeight:600}}>{$(c.total)}</span></div>;})}
          <div style={{borderTop:"1px solid rgba(255,255,255,.08)",marginTop:8,paddingTop:8,display:"flex",justifyContent:"space-between"}}>
            <span style={{fontSize:13,color:"#0f172a",fontWeight:600}}>Margen estimado</span>
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:14,flexWrap:"wrap"}}>
        {["lista","estadisticas"].map(t=><button key={t} onClick={()=>setTab(t)} style={{padding:"7px 16px",borderRadius:8,border:"none",background:tab===t?"#d1fae5":"#f1f5f9",color:tab===t?VERDE:"#64748b",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:"inherit"}}>{t==="lista"?"≡ Lista":"∿ Historial"}</button>)}
      ...
      {tab==="lista"&&(
        <Crd>
          ...
          {ventas.length===0?(<div style={{textAlign:"center",padding:"48px 0"}}><p style={{fontSize:32,marginBottom:10}}>📋</p><p style={{color:"#94a3b8",fontSize:14,fontWeight:600}}>No hay ventas cargadas</p><button style={{...btnPri,marginTop:12}} onClick={()=>setModal("form")}>+ Cargar primera venta</button></div>):(
            <div style={{overflowX:"auto"}}>
              ...
              <p style={{fontSize:11,color:"#94a3b8",margin:"10px 4px 0",textAlign:"right"}}>{filt.length} de {ventas.length} registros</p>
            </div>
          )}
        </Crd>
      )}
      {tab==="estadisticas"&&histData.length>0&&(
        <Crd>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:10}}>
            <div><p style={{fontSize:14,fontWeight:700,color:"#0f172a",margin:"0 0 2px"}}>Facturación histórica mensual</p><p style={{fontSize:11,color:"#94a3b8",margin:0}}>Barras = mes · Línea azul = acumulado · 🏆 mejor mes</p></div>
            <div style={{display:"flex",gap:20}}><div style={{textAlign:"right"}}><p style={{fontSize:10,color:"#475569",margin:"0 0 2px",textTransform:"uppercase",fontWeight:700}}>Promedio mensual</p><p style={{fontSize:15,fontWeight:700,color:"#0f172a",margin:0}}>{$(prom)}</p></div></div>
          </div>
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={histData} margin={{top:10,right:55,left:0,bottom:0}}>
              ...
              <YAxis yAxisId="a" orientation="right" tick={{fill:"#94a3b8",fontSize:10}} axisLine={false} tickLine={false} tickFormatter={n=>n>=1e6?"$"+(n/1e6).toFixed(1)+"M":"$"+(n/1e3).toFixed(0)+"K"}/>
              ...
              <Bar yAxisId="m" dataKey="vendido" name="vendido" radius={[5,5,0,0]}>
                {histData.map((d,i)=><Cell key={i} fill={i===histData.length-1?"#059669":d.vendido===Math.max(...histData.map(x=>x.vendido))?"#fbbf24":VERDE}/>)}
              </Bar>
            ...
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              ...
              <tbody>{histData.map((d,i)=>{const vp=prom?((d.vendido-prom)/prom*100):0;const mejor=d.vendido===Math.max(...histData.map(x=>x.vendido));return(
                <tr key={i} style={{background:mejor?"rgba(251,191,36,.04)":"transparent"}}>
                  ...
                  <Td><span style={{background:d.vendido&&d.cobrado/d.vendido>.9?"#d1fae5":"#fef3c7",color:d.vendido&&d.cobrado/d.vendido>.9?VERDE:AMBAR,padding:"1px 7px",borderRadius:20,fontSize:11,fontWeight:600}}>{d.vendido?pct(d.cobrado/d.vendido*100):"—"}</span></Td>
    ...
    <div>
      ...
      <div style={{display:"flex",gap:10,marginBottom:14,flexWrap:"wrap"}}>
        ...
        <div style={{display:"flex",gap:6}}>
          {["lista","cards"].map(t=><button key={t} onClick={()=>setCards(t==="cards")} style={{padding:"7px 14px",borderRadius:8,border:"none",background:(t==="cards")===cards?"#d1fae5":"#f1f5f9",color:(t==="cards")===cards?VERDE:"#64748b",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:"inherit"}}>{t==="lista"?"≡":"⊞"}</button>)}
        </div>
      </div>
      {clientes.length===0?(<Crd><div style={{textAlign:"center",padding:"48px 0"}}><p style={{fontSize:32,marginBottom:10}}>👥</p><p style={{color:"#94a3b8",fontSize:14}}>Los clientes aparecen automáticamente al cargar ventas.</p></div></Crd>):!cards?(
        <Crd>
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              ...
              <tbody>{filt.map(c=>(
                <tr key={c.empresa} style={{cursor:"pointer"}} onClick={()=>{setSel(c);setModal("det");}}>
                  <Td><div style={{display:"flex",alignItems:"center",gap:8}}><Av name={c.empresa} size={28}/><span style={{fontWeight:600,color:"#0f172a"}}>{c.empresa}</span></div></Td>
                  <Td>{c.extra?.contacto||<span style={{color:"#94a3b8",fontSize:11}}>sin datos</span>}</Td>
          ...
          {filt.map(c=>(
            <div key={c.empresa} className="pcard" onClick={()=>{setSel(c);setModal("det");}}>
              <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:10}}>
                <div style={{display:"flex",alignItems:"center",gap:10}}><Av name={c.empresa} size={38}/><div><p style={{fontSize:13,fontWeight:700,color:"#0f172a",margin:0}}>{c.empresa}</p><p style={{fontSize:11,color:"#475569",margin:"2px 0 0"}}>{c.extra?.rubro||"Sin rubro"}</p></div></div>
              ...
              <div style={{borderTop:"1px solid rgba(255,255,255,.07)",paddingTop:10,marginTop:8,display:"flex",justifyContent:"space-between"}}>
                ...
                <div style={{textAlign:"right"}}><p style={{fontSize:10,color:"#475569",margin:0,textTransform:"uppercase",fontWeight:700}}>Facturas</p><p style={{fontSize:14,fontWeight:700,color:"#64748b",margin:0}}>{c.cvs}</p></div>
              </div>
              {c.tp>0&&<div style={{marginTop:8,padding:"5px 10px",background:"#fef2f2",borderRadius:7,display:"flex",justifyContent:"space-between"}}><span style={{fontSize:11,color:ROJO}}>Pendiente</span><span style={{fontSize:12,fontWeight:700,color:ROJO}}>{$(c.tp)}</span></div>}
            </div>
          ))}
        </div>
      )}
      {modal==="det"&&sel&&(
        <Mdl title="Ficha de cliente" onClose={()=>setModal(null)} wide>
          <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:20,padding:"16px",background:"#f8fafc",borderRadius:12,border:"1px solid rgba(255,255,255,.07)"}}>
            <Av name={sel.empresa} size={52}/><div style={{flex:1}}><h2 style={{color:"#0f172a",fontSize:17,fontWeight:700,margin:"0 0 4px"}}>{sel.empresa}</h2>{sel.extra?.rubro&&<p style={{color:"#64748b",fontSize:12,margin:0}}>{sel.extra.rubro}{sel.extra?.ciudad?` · ${sel.extra.ciudad}`:""}</p>}</div>
          ...
          {sel.extra?.notas&&<div style={{marginBottom:16,padding:"10px",background:"#fffbeb",border:"1px solid rgba(245,158,11,.15)",borderRadius:10}}><p style={{fontSize:11,color:"#78350f",margin:"0 0 4px"}}>NOTAS</p><p style={{fontSize:13,color:"#fcd34d",margin:0}}>{sel.extra.notas}</p></div>}
...
function PageProveedores(){
  ...
  return (
    <div>
      ...
      {venc>0&&<div style={{background:"#fef2f2",border:"1px solid rgba(248,113,113,.2)",borderRadius:12,padding:"12px 16px",marginBottom:16,display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:18}}>⚠️</span><p style={{color:"#fca5a5",fontSize:13,margin:0,fontWeight:600}}>{venc} factura{venc>1?"s":""} vencida{venc>1?"s":""}</p></div>}
      <div style={{display:"flex",gap:6,marginBottom:14,flexWrap:"wrap"}}>
        {["provs","facs"].map(t=><button key={t} onClick={()=>setTab(t)} style={{padding:"7px 16px",borderRadius:8,border:"none",background:tab===t?"#d1fae5":"#f1f5f9",color:tab===t?VERDE:"#64748b",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:"inherit"}}>{t==="provs"?`🏭 Proveedores (${provs.length})`:`📄 Facturas (${facs.length})`}</button>)}
        <div style={{marginLeft:"auto",display:"flex",gap:8}}>
          ...
          <button style={{...btnPri,background:"#ecfdf5",borderColor:"rgba(16,185,129,.4)",color:"#059669"}} onClick={()=>{setSelFc(null);setModal("fc");}}>+ Factura</button>
      ...
      {tab==="provs"&&(
        <Crd>
          {provs.length===0?(<div style={{textAlign:"center",padding:"48px 0"}}><p style={{fontSize:32,marginBottom:10}}>🏭</p><p style={{color:"#94a3b8",fontSize:14,fontWeight:600}}>No hay proveedores cargados</p><button style={{...btnPri,marginTop:12}} onClick={()=>setModal("prov")}>+ Agregar proveedor</button></div>):(
            <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}>
              ...
              <tbody>{filtP.map(p=>(
                <tr key={p.id} style={{cursor:"pointer"}} onClick={()=>{setSel(p);setModal("detP");}}>
                  <Td><div style={{display:"flex",alignItems:"center",gap:8}}><Av name={p.nombre} size={28}/><span style={{fontWeight:600,color:"#0f172a"}}>{p.nombre}</span></div></Td>
      ...
      {tab==="facs"&&(
        <Crd>
          {facs.length===0?(<div style={{textAlign:"center",padding:"48px 0"}}><p style={{fontSize:32,marginBottom:10}}>📄</p><p style={{color:"#94a3b8",fontSize:14,fontWeight:600}}>No hay facturas de proveedor</p><button style={{...btnPri,marginTop:12}} onClick={()=>setModal("fc")}>+ Cargar factura</button></div>):(
            <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}>
              ...
              <tbody>{filtF.map(f=>{const v=f.ventaId?ventas.find(x=>x.id===f.ventaId):null;const venc2=f.estado==="Pendiente"&&f.fechaVto&&f.fechaVto<hoyS;return(
                <tr key={f.id}>
                  ...
                  <Td>{v?<span style={{background:"#d1fae5",color:VERDE,padding:"2px 8px",borderRadius:20,fontSize:11,fontWeight:600}}>{v.cliente}</span>:<span style={{color:"#94a3b8",fontSize:11}}>—</span>}</Td>
  ...
  return (
    <div>
      <div style={{display:"flex",gap:6,marginBottom:16}}>
        {["gastos","pagos"].map(t=><button key={t} onClick={()=>setTab(t)} style={{padding:"9px 22px",borderRadius:9,border:tab===t?"1px solid rgba(16,185,129,.25)":"1px solid transparent",background:tab===t?"#d1fae5":"transparent",color:tab===t?VERDE:"#475569",cursor:"pointer",fontSize:14,fontWeight:700,fontFamily:"inherit"}}>{t==="gastos"?"💸 Gastos":"💳 Pagos / Cobros"}</button>)}
      </div>
      {tab==="gastos"&&(
        <div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
          ...
          <Crd>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
              <p style={{color:"#0f172a",fontWeight:700,fontSize:14,margin:0}}>Registro de gastos</p>
            ...
            {gastos.length===0?(<div style={{textAlign:"center",padding:"48px 0"}}><p style={{fontSize:32,marginBottom:10}}>💸</p><p style={{color:"#94a3b8",fontSize:14,fontWeight:600}}>No hay gastos registrados</p><button style={{...btnPri,marginTop:12}} onClick={()=>setModal("gasto")}>+ Cargar primer gasto</button></div>):(
              <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}>
                ...
                <tbody>{filtG.map(g=>{const v=g.ventaId?ventas.find(x=>x.id===g.ventaId):null;return(
                  <tr key={g.id}>
                    ...
                    <Td>{v?<span style={{background:"#d1fae5",color:VERDE,padding:"2px 8px",borderRadius:20,fontSize:10,fontWeight:600}}>{v.cliente}</span>:<span style={{color:"#94a3b8",fontSize:11}}>—</span>}</Td>
                    <td style={{padding:"9px 8px",borderBottom:"1px solid rgba(255,255,255,.04)"}}>
                      <div className="ra" style={{display:"flex",gap:4}}>
              ...
              </table><p style={{fontSize:11,color:"#94a3b8",margin:"10px 4px 0",textAlign:"right"}}>{filtG.length} registros · Total: <strong style={{color:ROJO}}>{$(filtG.reduce((a,g)=>a+pN(g.total),0))}</strong></p></div>
            )}
          </Crd>
        </div>
      )}
        ...
        <div>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:10}}>
            <div style={{display:"flex",gap:4}}>
              {["cheques","transfs"].map(t=><button key={t} onClick={()=>setSubTab(t)} style={{padding:"7px 14px",borderRadius:8,border:"none",background:subTab===t?"#d1fae5":"#f1f5f9",color:subTab===t?VERDE:"#64748b",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:"inherit"}}>{t==="cheques"?`🧾 Cheques (${cheques.length})`:`↔ Transferencias (${transfs.length})`}</button>)}
            </div>
            <div style={{display:"flex",gap:8}}>
              <button style={{...btnPri,background:"#ecfdf5",borderColor:"rgba(16,185,129,.4)",color:"#059669"}} onClick={()=>{setSel(null);setModal("transf");}}>+ Transferencia</button>
          ...
          {subTab==="cheques"&&(
            <Crd>
              {cheques.length===0?(<div style={{textAlign:"center",padding:"48px 0"}}><p style={{fontSize:32,marginBottom:10}}>🧾</p><p style={{color:"#94a3b8",fontSize:14}}>No hay cheques registrados</p><button style={{...btnPri,marginTop:12}} onClick={()=>setModal("cheque")}>+ Registrar cheque</button></div>):(
                <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}>
          ...
          {subTab==="transfs"&&(
            <Crd>
              {transfs.length===0?(<div style={{textAlign:"center",padding:"48px 0"}}><p style={{fontSize:32,marginBottom:10}}>↔</p><p style={{color:"#94a3b8",fontSize:14}}>No hay transferencias registradas</p><button style={{...btnPri,marginTop:12}} onClick={()=>setModal("transf")}>+ Registrar transferencia</button></div>):(
                <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}>
                  ...
                  <tbody>{filtTr.map(t=>{const v=t.ventaId?ventas.find(x=>x.id===t.ventaId):null;return(
                    <tr key={t.id}>
                      <Td>{fecha(t.fecha)}</Td>
                      <Td><span style={{background:t.subtipo==="cobro"?"#d1fae5":"#fee2e2",color:t.subtipo==="cobro"?VERDE:ROJO,padding:"2px 9px",borderRadius:20,fontSize:11,fontWeight:600}}>{t.subtipo==="cobro"?"↓ Cobro":"↑ Pago"}</span></Td>
                ...
                </table><p style={{fontSize:11,color:"#94a3b8",margin:"10px 4px 0",textAlign:"right"}}>Entradas: <strong style={{color:VERDE}}>{$(tEnt)}</strong> · Salidas: <strong style={{color:ROJO}}>{$(tSal)}</strong> · Neto: <strong style={{color:tEnt-tSal>=0?VERDE:ROJO}}>{$(tEnt-tSal)}</strong></p></div>
              )}
            </Crd>
          )}
        </div>
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:14}}>
        {["res","movs","proy"].map(t=><button key={t} onClick={()=>setTab(t)} style={{padding:"7px 16px",borderRadius:8,border:"none",background:tab===t?"#d1fae5":"#f1f5f9",color:tab===t?VERDE:"#64748b",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:"inherit"}}>{t==="res"?"📊 Resumen":t==="movs"?"↔ Movimientos":"🔮 Proyección"}</button>)}
        <div style={{marginLeft:"auto",display:"flex",gap:8}}>
          <button style={{...btnPri,background:"#ecfdf5",borderColor:"rgba(16,185,129,.4)",color:"#059669"}} onClick={()=>{setSel(null);setModal("mov");}}>+ Movimiento</button>
          <button style={btnPri} onClick={()=>{setSel(null);setModal("cuenta");}}>+ Cuenta</button>
        </div>
      </div>
      {tab==="res"&&(
        <div>
          {cuentas.length===0?(<Crd><div style={{textAlign:"center",padding:"40px 0"}}><p style={{fontSize:32,marginBottom:10}}>🏦</p><p style={{color:"#94a3b8",fontSize:14,fontWeight:600}}>No hay cuentas registradas</p><button style={{...btnPri,marginTop:12}} onClick={()=>setModal("cuenta")}>+ Agregar primera cuenta</button></div></Crd>):(
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:12,marginBottom:16}}>
              {cuentas.map(c=>(
                <div key={c.id} style={{background:"#ffffff",borderRadius:14,padding:"18px 20px",border:`1px solid ${c.tipo==="FCI"?"rgba(16,185,129,.25)":c.moneda!=="ARS"?"rgba(245,158,11,.2)":"#e2e8f0"}`,position:"relative"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                    <div><p style={{fontSize:10,color:"#475569",margin:"0 0 2px",textTransform:"uppercase",fontWeight:700}}>{c.tipo}</p><p style={{fontSize:14,fontWeight:700,color:"#0f172a",margin:0}}>{c.nombre}</p>{c.banco&&<p style={{fontSize:11,color:"#475569",margin:"2px 0 0"}}>{c.banco}</p>}</div>
                    <div style={{display:"flex",gap:4}}>
                  ...
                  <p style={{fontSize:22,fontWeight:700,color:pN(c.saldo)<0?ROJO:c.tipo==="FCI"?VERDE:c.moneda!=="ARS"?AMBAR:"#0f172a",margin:"0 0 4px"}}>{c.moneda!=="ARS"?c.moneda+" ":""}{$(c.saldo)}</p>
                  <p style={{fontSize:10,color:"#94a3b8",margin:0}}>Act.: {fecha(c.fechaActualizacion||hoyS)}</p>
                </div>
              ))}
            </div>
          )}
          {barData.length>0&&(
            <Crd>
              <p style={{color:"#0f172a",fontWeight:700,fontSize:14,marginBottom:14}}>Ingresos vs egresos por mes</p>
          ...
          {reales.length>0&&(
            <Crd mb={0}>
              <p style={{color:"#0f172a",fontWeight:700,fontSize:14,marginBottom:14}}>Últimos movimientos</p>
              {[...reales].sort((a,b)=>(b.fecha||"").localeCompare(a.fecha||"")).slice(0,6).map((m,i)=>{const c=cuentas.find(x=>x.id===m.cuentaId);return(
                <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"9px 0",borderBottom:i<5?"1px solid rgba(255,255,255,.05)":"none"}}>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    <div style={{width:30,height:30,borderRadius:8,background:m.esEgreso?"#fee2e2":"#d1fae5",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,flexShrink:0}}>{m.esEgreso?"↑":"↓"}</div>
                    <div><p style={{fontSize:13,color:"#0f172a",margin:0,fontWeight:500}}>{m.descripcion}</p><p style={{fontSize:11,color:"#475569",margin:0}}>{fecha(m.fecha)}{c?" · "+c.nombre:""}</p></div>
                  </div>
      ...
      {tab==="movs"&&(
        <Crd>
          ...
          {reales.length===0?(<div style={{textAlign:"center",padding:"48px 0"}}><p style={{fontSize:32,marginBottom:10}}>↔</p><p style={{color:"#94a3b8",fontSize:14}}>No hay movimientos registrados</p><button style={{...btnPri,marginTop:12}} onClick={()=>setModal("mov")}>+ Registrar primer movimiento</button></div>):(
            <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}>
              ...
              <tbody>{filtM.map(m=>{const c=cuentas.find(x=>x.id===m.cuentaId);return(
                <tr key={m.id}>
                  <td style={{padding:"8px 10px",borderBottom:"1px solid rgba(255,255,255,.04)",width:32}}><div style={{width:28,height:28,borderRadius:7,background:m.esEgreso?"#fee2e2":"#d1fae5",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13}}>{m.esEgreso?"↑":"↓"}</div></td>
        ...
        <div>
          {proyData.length>1&&(
            <Crd>
              ...
              <p style={{color:"#0f172a",fontWeight:700,fontSize:14,marginBottom:14}}>Evolución proyectada</p>
              <ResponsiveContainer width="100%" height={200}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <p style={{color:"#0f172a",fontWeight:700,fontSize:14,margin:0}}>Ítems proyectados ({proyect.length})</p>
            <div style={{display:"flex",gap:8}}>
          ...
          <Crd mb={0}>
            {proyect.length===0?(<div style={{textAlign:"center",padding:"36px 0"}}><p style={{fontSize:28,marginBottom:8}}>🔮</p><p style={{color:"#94a3b8",fontSize:13}}>Agregá ingresos y egresos futuros para ver la proyección</p></div>):(
              proyect.map(m=>(
                <div key={m.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"9px 0",borderBottom:"1px solid rgba(255,255,255,.05)"}}>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    <div style={{width:28,height:28,borderRadius:7,background:m.esEgreso?"#fee2e2":"#d1fae5",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,flexShrink:0}}>{m.esEgreso?"↑":"↓"}</div>
                    <div><p style={{fontSize:13,color:"#0f172a",margin:0}}>{m.descripcion}</p><p style={{fontSize:11,color:"#475569",margin:0}}>{fecha(m.fecha)}</p></div>
                  </div>
    ...
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20,flexWrap:"wrap",gap:10}}>
        <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
          {SECS.map((s,i)=><button key={s} onClick={()=>setSec(s)} style={{padding:"8px 16px",borderRadius:9,border:sec===s?"1px solid rgba(16,185,129,.25)":"1px solid transparent",background:sec===s?"#d1fae5":"transparent",color:sec===s?VERDE:"#475569",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:"inherit"}}>{SLBL[i]}</button>)}
        </div>
      ...
      {sec==="gral"&&(
        <div>
          ...
          {vsData.length>0&&(
            <Crd>
              <p style={{color:"#0f172a",fontWeight:700,fontSize:14,marginBottom:14}}>Ventas vs Gastos por mes</p>
              <ResponsiveContainer width="100%" height={200}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
            <Crd mb={0}>
              <p style={{color:"#0f172a",fontWeight:700,fontSize:14,marginBottom:14}}>Estados de ventas</p>
              {estData.length>0?(
                <div style={{display:"flex",alignItems:"center",gap:16}}>
                  ...
                  <div style={{flex:1}}>{estData.map((d,i)=><div key={d.name} style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><div style={{display:"flex",alignItems:"center",gap:6}}><div style={{width:9,height:9,borderRadius:2,background:COLORS[i%COLORS.length],flexShrink:0}}/><span style={{fontSize:12,color:"#64748b"}}>{d.name}</span></div><span style={{fontSize:12,fontWeight:700,color:"#0f172a"}}>{d.value}</span></div>)}</div>
                </div>
              ):<p style={{color:"#94a3b8",fontSize:13,textAlign:"center",padding:"20px 0"}}>Sin datos</p>}
            </Crd>
            <Crd mb={0}>
              <p style={{color:"#0f172a",fontWeight:700,fontSize:14,marginBottom:14}}>Gastos por categoría</p>
              {catData.length>0?(
                <div style={{display:"flex",alignItems:"center",gap:16}}>
                  ...
                  <div style={{flex:1}}>{catData.slice(0,6).map((d,i)=><div key={d.name} style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><div style={{display:"flex",alignItems:"center",gap:6}}><div style={{width:9,height:9,borderRadius:2,background:COLORS[i%COLORS.length],flexShrink:0}}/><span style={{fontSize:11,color:"#64748b",maxWidth:100,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{d.name}</span></div><span style={{fontSize:11,fontWeight:700,color:ROJO}}>{$(d.value)}</span></div>)}</div>
                </div>
              ):<p style={{color:"#94a3b8",fontSize:13,textAlign:"center",padding:"20px 0"}}>Sin datos</p>}
            </Crd>
          </div>
          ...
          {vMesArr.length>0&&(()=>{let a=0;const hd=vMesArr.map(d=>{a+=d.vendido;return{...d,l:mesL(d.mes),acum:Math.round(a)};});const pm=Math.round(tv/vMesArr.length);const mejor=Math.max(...hd.map(d=>d.vendido));return(
            <Crd>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:14,flexWrap:"wrap",gap:10}}>
                <div><p style={{fontSize:14,fontWeight:700,color:"#0f172a",margin:"0 0 2px"}}>Facturación histórica mensual</p><p style={{fontSize:11,color:"#94a3b8",margin:0}}>Barras = mes · Línea = acumulado · 🏆 mejor</p></div>
                <div style={{textAlign:"right"}}><p style={{fontSize:10,color:"#475569",margin:"0 0 2px",textTransform:"uppercase",fontWeight:700}}>Promedio mensual</p><p style={{fontSize:15,fontWeight:700,color:"#0f172a",margin:0}}>{$(pm)}</p></div>
              </div>
              <ResponsiveContainer width="100%" height={240}>
      ...
      {sec==="finanzas"&&(
        <div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
            <Crd mb={0}>
              <p style={{color:"#0f172a",fontWeight:700,fontSize:14,marginBottom:14}}>Gastos por categoría</p>
              {catData.map((d,i)=>(<div key={d.name} style={{marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:13,color:"#94a3b8"}}>{d.name}</span><span style={{fontSize:13,fontWeight:700,color:COLORS[i%COLORS.length]}}>{$(d.value)}</span></div><div style={{height:5,background:"#e2e8f0",borderRadius:3}}><div style={{height:"100%",width:Math.round(d.value/catData[0]?.value*100)+"%",background:COLORS[i%COLORS.length],borderRadius:3}}/></div></div>))}
            </Crd>
            <Crd mb={0}>
              <p style={{color:"#0f172a",fontWeight:700,fontSize:14,marginBottom:14}}>Estado facturas proveedores</p>
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {[{l:"Total facturado",v:$(tfp),c:ROJO},{l:"Total pagado",v:$(pagfp),c:VERDE},{l:"Pendiente pago",v:$(tfp-pagfp),c:AMBAR}].map(x=>(<div key={x.l} style={{padding:"12px 14px",background:"#f8fafc",borderRadius:10,border:"1px solid rgba(255,255,255,.06)"}}><p style={{fontSize:11,color:"#475569",margin:"0 0 4px",textTransform:"uppercase",fontWeight:700}}>{x.l}</p><p style={{fontSize:20,fontWeight:700,color:x.c,margin:0}}>{x.v}</p></div>))}
              </div>
            </Crd>
          </div>
        </div>
      ...
      {sec==="clientes"&&(
        <div>
          ...
          <Crd mb={16}>
            <p style={{color:"#0f172a",fontWeight:700,fontSize:14,marginBottom:14}}>Top clientes por facturación</p>
            {topCli.map(x=>(<div key={x.n} style={{marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}><div><span style={{fontSize:13,color:"#0f172a",fontWeight:600}}>{x.n}</span><span style={{fontSize:11,color:"#475569",marginLeft:8}}>{x.fc} factura{x.fc>1?"s":""}</span></div><div style={{textAlign:"right"}}><span style={{fontSize:13,fontWeight:700,color:VERDE}}>{$(x.tv)}</span>{x.tv>x.tc&&<span style={{fontSize:11,color:ROJO,marginLeft:8}}>({$(x.tv-x.tc)} pend.)</span>}</div></div><div style={{height:5,background:"#e2e8f0",borderRadius:3}}><div style={{height:"100%",width:Math.round(x.tv/topCli[0].tv*100)+"%",background:VERDE,borderRadius:3}}/></div></div>))}
          </Crd>
          <Crd mb={0}>
            <p style={{color:"#0f172a",fontWeight:700,fontSize:14,marginBottom:14}}>Detalle por cliente</p>
            <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr><Th>Cliente</Th><Th>Facturas</Th><Th>Total vendido</Th><Th>Total cobrado</Th><Th>Pendiente</Th><Th>% cobrado</Th></tr></thead><tbody>{topCli.map(x=>(<tr key={x.n}><Td fw>{x.n}</Td><Td>{x.fc}</Td><Td g fw>{$(x.tv)}</Td><Td>{$(x.tc)}</Td><Td r={x.tv>x.tc} fw>{$(x.tv-x.tc)}</Td><Td><span style={{background:x.tv&&x.tc/x.tv>.9?"#d1fae5":"#fef3c7",color:x.tv&&x.tc/x.tv>.9?VERDE:AMBAR,padding:"2px 8px",borderRadius:20,fontSize:11,fontWeight:600}}>{x.tv?pct(x.tc/x.tv*100):"—"}</span></Td></tr>))}</tbody></table></div>
          </Crd>
        </div>
      )}
      {sec==="rentabilidad"&&(
        <div>
          ...
          {rent.length>0?(
            <Crd>
              <p style={{color:"#0f172a",fontWeight:700,fontSize:14,marginBottom:14}}>Rentabilidad por operación</p>
              <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr><Th>Cliente</Th><Th>Fecha</Th><Th>Venta s/IVA</Th><Th>Costos</Th><Th>Margen $</Th><Th>Margen %</Th></tr></thead><tbody>{rent.map((r,i)=>(<tr key={i}><Td fw>{r.c}</Td><Td>{fecha(r.d)}</Td><Td>{$(r.v)}</Td><Td r>{$(r.co)}</Td><Td fw><span style={{color:r.m>=0?VERDE:ROJO}}>{$(r.m)}</span></Td><Td><span style={{background:r.p>25?"#d1fae5":r.p>10?"#fef3c7":"#fee2e2",color:r.p>25?VERDE:r.p>10?AMBAR:ROJO,padding:"2px 8px",borderRadius:20,fontSize:11,fontWeight:700}}>{pct(r.p)}</span></Td></tr>))}</tbody></table></div>
            </Crd>
          ):<Crd><div style={{textAlign:"center",padding:"36px 0"}}><p style={{fontSize:28,marginBottom:8}}>📊</p><p style={{color:"#94a3b8",fontSize:13}}>Asociá facturas de proveedor y gastos a tus ventas para ver la rentabilidad por operación.</p></div></Crd>}
        </div>
      )}
    </div>
  );
...
function PageCotizaciones(){
  ...
  return (
    <div>
      ...
      <Crd>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <p style={{color:"#0f172a",fontWeight:700,fontSize:14,margin:0}}>Pipeline de cotizaciones</p>
        ...
        {cots.length===0?(<div style={{textAlign:"center",padding:"48px 0"}}><p style={{fontSize:32,marginBottom:10}}>📋</p><p style={{color:"#94a3b8",fontSize:14,fontWeight:600}}>No hay cotizaciones cargadas</p><button style={{...btnPri,marginTop:12}} onClick={()=>setModal("form")}>+ Registrar primer oportunidad</button></div>):(
          <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}>
          ...
          </table><p style={{fontSize:11,color:"#94a3b8",margin:"10px 4px 0",textAlign:"right"}}>{filt.length} de {cots.length}</p></div>
        )}
      </Crd>
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:20}}>
        {["inicio","mapear","done"].map((s,i)=>(
          <div key={s} style={{display:"flex",alignItems:"center",gap:8}}>
            <div style={{width:28,height:28,borderRadius:"50%",background:step===s||( s==="done"&&step==="cargando")?"#065f46":"#f1f5f9",border:`1px solid ${step===s?"#10b981":"#e2e8f0"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:step===s?"#059669":"#475569"}}>{i+1}</div>
            <span style={{fontSize:12,color:step===s?"#059669":"#475569",fontWeight:step===s?600:400}}>{s==="inicio"?"Seleccionar":s==="mapear"?"Mapear columnas":"Resultado"}</span>
      ...
      {step==="inicio"&&(
        <div>
          <Crd>
            <p style={{color:"#0f172a",fontWeight:700,fontSize:14,marginBottom:16}}>1. Seleccioná el módulo de destino</p>
            <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10,marginBottom:24}}>
              {Object.entries(IMP_MODULES).map(([key,m])=>(
                <button key={key} onClick={()=>setModKey(key)} style={{padding:"12px 16px",borderRadius:12,border:`1px solid ${modKey===key?m.color:"#e2e8f0"}`,background:modKey===key?`rgba(${modKey===key&&m.color==="#10b981"?"16,185,129":m.color==="#3b82f6"?"59,130,246":m.color==="#f87171"?"248,113,113":"245,158,11"},.08)`:"#f8fafc",cursor:"pointer",textAlign:"left",fontFamily:"inherit",transition:"all .15s"}}>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    <span style={{fontSize:22}}>{m.icon}</span>
                    <div>
                      <p style={{color:modKey===key?m.color:"#0f172a",fontWeight:600,fontSize:13,margin:0}}>{m.label}</p>
            ...
            <p style={{color:"#0f172a",fontWeight:700,fontSize:14,marginBottom:12}}>2. Subí tu archivo Excel o CSV</p>
            <div
              ...
              <p style={{color:"#0f172a",fontWeight:600,fontSize:14,margin:"0 0 6px"}}>Arrastrá tu archivo acá</p>
              <p style={{color:"#475569",fontSize:13,margin:"0 0 14px"}}>o hacé clic para seleccionar</p>
              <div style={{display:"flex",gap:8,justifyContent:"center"}}>
                {[".xlsx",".xls",".csv"].map(ext=><span key={ext} style={{background:"#f1f5f9",padding:"3px 10px",borderRadius:20,fontSize:12,color:"#64748b"}}>{ext}</span>)}
              </div>
            </div>
          </Crd>
          {/* Formato esperado */}
          <Crd mb={0}>
            <p style={{color:"#0f172a",fontWeight:700,fontSize:13,marginBottom:12}}>📋 Formato esperado para <span style={{color:mod.color}}>{mod.label}</span></p>
            <div style={{overflowX:"auto"}}>
              <table style={{borderCollapse:"collapse",fontSize:12}}>
                ...
                <tbody><tr>{mod.fields.map(f=><td key={f.id} style={{padding:"6px 12px",color:"#94a3b8",fontSize:11,whiteSpace:"nowrap"}}>{f.hint||"—"}</td>)}</tr></tbody>
              </table>
            </div>
            <p style={{fontSize:11,color:"#94a3b8",marginTop:8}}>★ Campos obligatorios · La primera fila debe ser el encabezado · El mapeo de columnas es automático</p>
          </Crd>
        </div>
      )}
        ...
        <div>
          <Crd>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:18,flexWrap:"wrap",gap:10}}>
              <div>
                <p style={{color:"#0f172a",fontWeight:700,fontSize:14,margin:"0 0 4px"}}>Mapeo de columnas — {mod.label}</p>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:18}}>
              {mod.fields.map(f=>(
                <div key={f.id} style={{display:"flex",flexDirection:"column",gap:4}}>
                  ...
                  {f.hint&&<span style={{fontSize:10,color:"#94a3b8"}}>{f.hint}</span>}
                  <select value={mapping[f.id]||""} onChange={e=>setMapping(p=>({...p,[f.id]:e.target.value}))} style={{...IS,border:`1px solid ${mapping[f.id]?VERDE:"#e2e8f0"}`}}>
            ...
            {/* Merge mode */}
            <div style={{padding:"12px 14px",background:"#f8fafc",borderRadius:10,border:"1px solid rgba(255,255,255,.07)",marginBottom:18}}>
              <p style={{fontSize:11,fontWeight:700,color:"#64748b",marginBottom:8,textTransform:"uppercase"}}>Modo de importación</p>
              <div style={{display:"flex",gap:10}}>
                {[{v:"append",l:"➕ Agregar",d:"Añade sin borrar lo existente"},{v:"replace",l:"🔄 Reemplazar",d:"Borra todo y reemplaza"}].map(opt=>(
                  <button key={opt.v} onClick={()=>setMergeMode(opt.v)} style={{flex:1,padding:"10px 12px",borderRadius:9,border:`1px solid ${mergeMode===opt.v?(opt.v==="replace"?"#ef4444":"#10b981"):"#e2e8f0"}`,background:mergeMode===opt.v?(opt.v==="replace"?"#fef2f2":"#ecfdf5"):"transparent",color:mergeMode===opt.v?(opt.v==="replace"?ROJO:VERDE):"#64748b",cursor:"pointer",textAlign:"left",fontFamily:"inherit"}}>
            ...
            <div style={{overflowX:"auto",maxHeight:200,overflowY:"auto"}}>
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                <thead style={{position:"sticky",top:0,background:"#ffffff"}}>
                  <tr>{mod.fields.filter(f=>mapping[f.id]).map(f=><th key={f.id} style={{textAlign:"left",padding:"6px 10px",fontSize:10,fontWeight:700,color:"#94a3b8",textTransform:"uppercase",borderBottom:"1px solid rgba(255,255,255,.07)",whiteSpace:"nowrap"}}>{f.label}</th>)}</tr>
                </thead>
                <tbody>
                  {sheetData.rows.slice(0,5).map((row,i)=>(
                    <tr key={i}>{mod.fields.filter(f=>mapping[f.id]).map(f=><td key={f.id} style={{padding:"6px 10px",borderBottom:"1px solid rgba(255,255,255,.04)",color:"#94a3b8",whiteSpace:"nowrap",maxWidth:150,overflow:"hidden",textOverflow:"ellipsis"}}>{row[mapping[f.id]]??""}</td>)}</tr>
                  ))}
                </tbody>
              </table>
            </div>
      ...
      {step==="cargando"&&(
        <div style={{textAlign:"center",padding:"80px 0"}}>
          ...
          <p style={{fontSize:17,fontWeight:700,color:"#0f172a",margin:"0 0 8px"}}>Importando datos...</p>
      ...
      {step==="done"&&result&&(
        <div>
          <div style={{background:result.ok>0?"#ecfdf5":"rgba(239,68,68,.06)",borderRadius:16,padding:"32px",border:`1px solid ${result.ok>0?"rgba(5,150,105,.15)":"rgba(239,68,68,.2)"}`,textAlign:"center",marginBottom:20}}>
            <p style={{fontSize:48,marginBottom:12}}>{result.ok>0?"✅":"❌"}</p>
            <p style={{fontSize:20,fontWeight:700,color:"#0f172a",margin:"0 0 8px"}}>{result.ok} de {result.total} registros importados correctamente</p>
    ...
    <div>
      ...
      {vendedores.length===0?(
        <Crd><div style={{textAlign:"center",padding:"48px 0"}}>
          ...
          <p style={{color:"#94a3b8",fontSize:14,fontWeight:600}}>No hay ventas registradas aún</p>
          <p style={{color:"#dbeafe",fontSize:13}}>Cargá ventas con el campo "Vendedor" completo para ver las comisiones.</p>
        </div></Crd>
      ):(
        comisiones.map(c=>(
          <Crd key={c.vnd}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16,flexWrap:"wrap",gap:10}}>
              <div style={{display:"flex",alignItems:"center",gap:12}}>
                ...
                <div>
                  <h3 style={{color:"#0f172a",fontSize:16,fontWeight:700,margin:"0 0 4px"}}>{c.vnd}</h3>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <span style={{background:"#d1fae5",color:VERDE,padding:"2px 10px",borderRadius:20,fontSize:12,fontWeight:600}}>{c.pct}% de comisión</span>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:16}}>
              {[
                {l:"Total cobrado",v:$(c.totalCobrado),c:"#0f172a"},
                {l:"Comisión total",v:$(c.total),c:VERDE},
                {l:"Ya liquidado",v:$(c.comLiq),c:"#64748b"},
              ].map((m,i)=>(
                <div key={i} style={{background:"#f8fafc",borderRadius:10,padding:"10px 14px",border:"1px solid rgba(255,255,255,.06)"}}>
                  ...
                  {c.vVnd.map(v=>{
                    ...
                    return(
                      <tr key={v.id}>
                        ...
                        <Td>
                          <span style={{background:liq?"#d1fae5":"#fef3c7",color:liq?VERDE:AMBAR,padding:"2px 9px",borderRadius:20,fontSize:11,fontWeight:600}}>
  ...
  return (
    <div style={{display:"flex",minHeight:"100vh",background:"#f8fafc",fontFamily:"'Inter',system-ui,sans-serif",color:"#0f172a"}}>
      ...
      <div style={{width:215,background:"#ffffff",borderRight:"1px solid rgba(255,255,255,.07)",display:"flex",flexDirection:"column",flexShrink:0,minHeight:"100vh"}}>
        ...
        <div style={{padding:10,borderTop:"1px solid rgba(255,255,255,.07)"}}>
          <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",background:"#f1f5f9",borderRadius:8}}>
            <div style={{width:28,height:28,borderRadius:"50%",background:"#065f46",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,color:"#059669",fontWeight:700,flexShrink:0}}>
              {user?.email?.[0]?.toUpperCase()||"U"}
            </div>
            <div style={{flex:1,minWidth:0}}>
              <p style={{color:"#0f172a",fontSize:11,fontWeight:700,margin:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{user?.email||"Usuario"}</p>
              <p style={{color:"#94a3b8",fontSize:10,margin:0}}>Administrador</p>
            </div>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{background:"#ffffff",borderBottom:"1px solid rgba(255,255,255,.07)",padding:"13px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          <h2 style={{color:"#0f172a",fontSize:17,fontWeight:700,margin:0}}>{LABELS[page]}</h2>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
